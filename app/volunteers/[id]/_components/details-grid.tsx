...
function InfoItem({ label, value }: { label: string; value?: string }) {
  return (
    <div>
      ...
      <p className="text-sm font-medium text-card-foreground">{value || "N/A"}</p>
    </div>
  );
}
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <motion.div custom={0} variants={cardVariant} initial="hidden" animate="visible">
        <Card className="h-full rounded-xl border bg-card/70 backdrop-blur-sm">
          ...
          <CardContent className="space-y-3">
            ...
            <InfoItem label="Date of Birth" value={formatDate(volunteer.dateOfBirth)} />
          </CardContent>
        </Card>
      </motion.div>
      ...
      <motion.div custom={2} variants={cardVariant} initial="hidden" animate="visible">
        <Card className="h-full rounded-xl border bg-card/70 backdrop-blur-sm">
          ...
          <CardContent className="space-y-3">
            ...
            <div>
              <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-1">Occupation</p>
            ...
            {volunteer.sacraments.length > 0 ? (
              <div className="flex flex-wrap gap-1.5">
                {volunteer.sacraments.map((s: any) => (
                  <Badge key={s} variant="secondary" className="rounded-md text-xs font-normal">
                    {s}
                  </Badge>
                ))}
              </div>
