...
export function FormationsTimelines({ volunteer }: { volunteer: Volunteer }) {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      >
        <Card className="rounded-xl border bg-card/70 backdrop-blur-sm h-full">
          ...
          <CardContent>
            {volunteer.formations.length > 0 ? (
              ...
              <p className="text-sm text-muted-foreground italic">No formations recorded</p>
            )}
          </CardContent>
        </Card>
      </motion.div>
        ...
        <Card className="rounded-xl border bg-card/70 backdrop-blur-sm h-full">
          ...
          <CardContent>
            {volunteer.timelines.length > 0 ? (
              <div className="relative pl-4 border-l-2 border-primary/20 space-y-4">
                {volunteer.timelines.map((t: any) => (
                  <div key={t.id} className="relative">
                    ...
                    <div>
                      <p className="text-sm font-medium text-card-foreground">{t.organization}</p>
              ...
              <p className="text-sm text-muted-foreground italic">No timelines recorded</p>
            )}
          </CardContent>
        </Card>
      </motion.div>
